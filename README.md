...
- **Always-on background activation** — Incoming CRM events are queued in Redis and immediately attempt a Modal webhook fire (`POST /api/agent/trigger`) so per-realtor agents can react in near real-time with queue-based fallback if Modal is unavailable.
