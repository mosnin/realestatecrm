...
import {
  DURABLE_SCHEDULE_OCCURRENCE_RUNTIME_BLOCKERS,
  claimOccurrence,
  durableScheduleOccurrenceRuntimeReadiness,
...
describe('durable schedule occurrences', () => {
  ...
  it('stays construction-only regardless of historical rollout flags', () => {
    expect(durableScheduleOccurrenceRuntimeReadiness()).toEqual({
      enabled: false,
      stage: 'construction_only',
      blockers: DURABLE_SCHEDULE_OCCURRENCE_RUNTIME_BLOCKERS,
    });
    expect(DURABLE_SCHEDULE_OCCURRENCE_RUNTIME_BLOCKERS).toEqual([
    ...
    vi.stubEnv('AGENT_RUN_POLICY_SECRET', 'x'.repeat(48));
    expect(durableScheduleOccurrencesEnabled()).toBe(false);
    vi.stubEnv('AGENT_RUN_POLICY_MODE', 'enforce');
    vi.stubEnv('AGENT_RUN_POLICY_SECRET', 'short');
    expect(durableScheduleOccurrencesEnabled()).toBe(false);
    vi.stubEnv('AGENT_RUN_POLICY_SECRET', 'x'.repeat(48));
    expect(durableScheduleOccurrencesEnabled()).toBe(false);
  });
