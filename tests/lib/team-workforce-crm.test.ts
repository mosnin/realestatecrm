...
vi.mock('server-only',()=>({}));
const { shared, execute }=vi.hoisted(()=>({shared:vi.fn(),execute:vi.fn()}));
...
describe('team workforce CRM catalog',()=>{
 it('advertises only the explicit shared-record reader',async()=>{
  const result=await queryWorkforceCrm(team,'clerk-a',{operation:'catalog'},signal);
  expect(result).toMatchObject({tools:[{name:'list_shared_records'}]});expect((result as {tools:unknown[]}).tools).toHaveLength(1);
 });
