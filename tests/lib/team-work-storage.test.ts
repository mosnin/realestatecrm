...
vi.mock('@/lib/supabase',()=>({supabase:{from(table:string){
 ...
 const q:Row={select:()=>q,eq:(key:string,value:unknown)=>{filters.push(row=>row[key]===value);return q;},in:(key:string,values:unknown[])=>{filters.push(row=>values.includes(row[key]));return q;},order:()=>q,range:(from:number,to:number)=>{range=[from,to];return q;},insert:(value:Row)=>{inserted=value;return q;},update:(value:Row)=>{patch=value;return q;},maybeSingle:async()=>({data:run()[0]??null,error:null}),single:async()=>({data:run()[0]??null,error:null}),then:(resolve:(value:unknown)=>unknown)=>Promise.resolve({data:run(),error:null}).then(resolve)};
 return q;
}}}));
import {listTeamWork,createTeamWork,updateTeamWork} from '@/lib/teams/work-items';
