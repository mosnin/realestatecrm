...
let shared=false;let fail=true;let brokerageAvailable=false;let brokerageShared=false;
const fetchMock=vi.fn(async (url:string,init?:RequestInit)=>{
 ...
 return new Response(JSON.stringify({records:shared?[{grantId:'grant-a',kind:'contact',title:'Alex',fields:{name:'Alex',email:'alex@example.test'},canRevoke:true,source:brokerageShared?'brokerage':undefined}]:[],brokerageSharingAvailable:brokerageAvailable,nextOffset:null}));
});
...
beforeEach(()=>{vi.useFakeTimers();vi.stubGlobal('React',React);vi.stubGlobal('IS_REACT_ACT_ENVIRONMENT',true);vi.stubGlobal('fetch',fetchMock);fetchMock.mockClear();shared=false;fail=true;brokerageAvailable=false;brokerageShared=false;host=document.createElement('div');document.body.append(host);root=createRoot(host);});
