...
# A separate, deliberately minimal image for the public-web Research
# Workspace. It does NOT inherit the full agent image or source tree: no DB,
...
# This secret must contain ONLY CHIPPI_BROWSER_APP_URL and
# CHIPPI_BROWSER_WORKER_SECRET. The public-web worker must never receive the
# broad chat/runtime secret bundle (database, model, integrations, etc.).
...
# ---------------------------------------------------------------------------
# Chippi Research Workspace — ephemeral public-web browser worker
# ---------------------------------------------------------------------------

@app.function(
    image=headless_image,
    secrets=browser_worker_secrets,
    timeout=900,
    max_containers=20,
)
async def run_headless_browser_session(session_id: str, lease_token: str) -> dict:
    """Run one fenced, fresh cloud browser for at most fifteen minutes.

    The Next.js side claims a durable BrowserSession lease before this worker
    is spawned. Each poll renews that lease; Stop, expiry, or a replacement
    worker makes the poll endpoint return stop=true. No shell, files, cookies,
    or login credentials are exposed to this process.
    """
    import logging
    import os
    import sys
    ...
    try:
        ...
        # Best effort only: an infrastructure kill is recovered by lease expiry.
        try:
...
@app.function(secrets=browser_worker_secrets, timeout=30)
@modal.fastapi_endpoint(method="POST", label="start-headless-browser")
async def start_headless_browser_workspace(item: dict) -> dict:
    """Accept a feature-on worker launch after the web app has claimed it."""
    import hmac
    import os
    import uuid
