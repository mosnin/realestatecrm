...
async def run_workspace(item: dict):
    ...
    if not validate_workspace_run_request(item, MAX_GOAL): return JSONResponse({"error":"invalid workspace request"}, status_code=400)
...
async def run_workspace_task(item: dict):
    ...
    if not validate_workspace_task_request(item, MAX_GOAL): return {"error": "invalid workspace task"}
    task_id, run_id, space_id, launch_token = str(item.get("task_id", "")), str(item.get("run_id", "")), str(item.get("space_id", "")), str(item.get("launch_token", ""))
    ...
    def event(kind: str, message: str, **extra):
        nonlocal seq
        current_sequence, seq = reserve_sequence(seq)
        reply = _task_callback({"task_id": task_id, "run_id": run_id, "space_id": space_id, "launch_token": launch_token, "sequence": current_sequence, "type": kind, "message": message, **extra})
        return reply.get("cancellationRequested", False)
    sandbox = None
    try:
...
async def launch_workspace_task(item: dict):
    ...
    expected = os.environ.get("CHIPPI_WORKSPACE_MODAL_SECRET", "")
    if not expected or not hmac.compare_digest(str(item.get("secret", "")), expected): return JSONResponse({"error": "Unauthorized"}, status_code=401)
    if not validate_workspace_task_request(item, MAX_GOAL): return JSONResponse({"error": "invalid workspace task"}, status_code=400)
