...
type WorkspaceFileSourceKind = 'root' | 'task';
...
export function WorkspaceRunPanel({
  ...
  const [workbenchFileError, setWorkbenchFileError] = useState<{ sourceKey: string; message: string } | null>(null);
  ...
  const openFileInWorkbench = async (fileId: string, sourceKind: WorkspaceFileSourceKind) => {
    if (!runId || !followUpsEnabled || !workbenchEnabled || !onOpenWorkbench || openingFileId) return;
    const sourceKey = `${sourceKind}:${fileId}`;
    setOpeningFileId(sourceKey); setWorkbenchFileError(null);
    try {
      const res = await fetch(`/api/workspace-runs/${encodeURIComponent(runId)}/files/${encodeURIComponent(fileId)}/workbench?slug=${encodeURIComponent(slug)}&sourceKind=${sourceKind}`, {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
      });
      const payload = await res.json().catch(() => ({})) as { artifactId?: unknown; versionNumber?: unknown; error?: unknown };
      if (!res.ok || typeof payload.artifactId !== 'string') {
        throw new Error(typeof payload.error === 'string' ? payload.error : 'Could not open this CSV in Workbench.');
      }
      onOpenWorkbench(payload.artifactId, typeof payload.versionNumber === 'number' ? payload.versionNumber : undefined);
    } catch (err) {
      setWorkbenchFileError({ sourceKey, message: err instanceof Error ? err.message : 'Could not open this CSV in Workbench.' });
    } finally {
  ...
  return <div className="flex h-full min-h-0 flex-col">
    ...
    <div className="min-h-0 flex-1 space-y-5 overflow-y-auto p-4">
      <section aria-labelledby="workspace-launch-status-title"><p id="workspace-launch-status-title" className="mb-2 text-[11px] font-medium uppercase tracking-wide text-muted-foreground">Launch continuity</p><p role="status" aria-atomic="true" className="text-xs text-muted-foreground">{workspaceLaunchMessage(run.status, launchReceipt)}</p>{run.status === 'failed' && run.error && <p role="alert" className="mt-1 text-[11px] text-destructive">{run.error}</p>}</section><section><p className="mb-2 text-[11px] font-medium uppercase tracking-wide text-muted-foreground">Live work</p><TerminalEvents events={run.events} empty="Waiting for the isolated workspace to start…" />{error && <p role="alert" className="mt-2 text-xs text-destructive">{error}</p>}</section>
...
function FileLinks({ sourceKind, runId, slug, files, empty, canOpenWorkbench = false, openingFileId, workbenchFileError, onOpenWorkbench }: { sourceKind: WorkspaceFileSourceKind; runId: string; slug: string; files: Array<{ id: string; fileId: string | null; name: string; mimeType: string; sizeBytes: number }>; empty?: string; canOpenWorkbench?: boolean; openingFileId?: string | null; workbenchFileError?: { sourceKey: string; message: string } | null; onOpenWorkbench?: (fileId: string, sourceKind: WorkspaceFileSourceKind) => void }) {
  if (!files.length) return empty ? <p className="text-xs text-muted-foreground">{empty}</p> : null;
  return <div className="space-y-1">{files.map((file) => {
    const csvEligible = canOpenWorkbench && file.fileId && file.mimeType === 'text/csv' && /\.csv$/i.test(file.name);
    const sourceKey = `${sourceKind}:${file.id}`;
    const isOpening = openingFileId === sourceKey;
    return <div key={file.id}><div className="flex items-center gap-1"><a href={file.fileId ? `/api/workspace-runs/${encodeURIComponent(runId)}/files/${encodeURIComponent(file.id)}?slug=${encodeURIComponent(slug)}` : undefined} className="flex min-w-0 flex-1 items-center gap-2 rounded-md px-2 py-1.5 text-xs hover:bg-muted">{file.name.endsWith('.csv') ? <Table2 className="size-3.5 shrink-0" /> : file.name.endsWith('.json') ? <Braces className="size-3.5 shrink-0" /> : <FileText className="size-3.5 shrink-0" />}<span className="truncate">{file.name}</span><span className="ml-auto shrink-0 text-muted-foreground">{Math.max(1, Math.ceil(file.sizeBytes / 1024))} KB</span></a>{csvEligible && <Button type="button" size="xs" variant="ghost" disabled={Boolean(openingFileId)} onClick={() => onOpenWorkbench?.(file.id, sourceKind)}>{isOpening ? <Loader2 className="mr-1 size-3 animate-spin" /> : <Table2 className="mr-1 size-3" />}Open in Workbench</Button>}</div>{workbenchFileError?.sourceKey === sourceKey && <p className="px-2 pb-1 text-xs text-destructive">{workbenchFileError.message}</p>}</div>;
  })}</div>;
}
