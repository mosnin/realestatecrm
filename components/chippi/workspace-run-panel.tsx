...
import { FileText, Loader2, Send, SquareTerminal, X } from 'lucide-react';
...
export function WorkspaceRunPanel({ runId, slug, onContinue, followUpsEnabled = false }: { runId: string | null; slug: string; onContinue?: () => void; followUpsEnabled?: boolean }) {
  ...
  return <div className="flex h-full min-h-0 flex-col">
    ...
    <div className="min-h-0 flex-1 space-y-5 overflow-y-auto p-4">
      ...
      {followUpsEnabled && run.status === 'completed' && <section className="border-t border-border/50 pt-4"><p className="text-sm font-medium">Continue this workspace</p><p className="mt-1 text-xs text-muted-foreground">Ask for a bounded private follow-up. Chippi opens a fresh isolated terminal with these files.</p><div className="mt-3 flex gap-2"><textarea value={instruction} onChange={(event) => setInstruction(event.target.value.slice(0, 1000))} disabled={taskActive || submitting} rows={3} placeholder="For example: summarize the pricing rationale for the seller review." className="min-w-0 flex-1 resize-none rounded-md border border-border bg-background px-3 py-2 text-xs outline-none placeholder:text-muted-foreground focus:ring-1 focus:ring-ring disabled:opacity-60" /><Button size="sm" className="self-end" disabled={taskActive || submitting || instruction.trim().length < 3} onClick={continueWorkspace}>{submitting ? <Loader2 className="size-3 animate-spin" /> : <Send className="size-3" />}<span className="sr-only">Continue workspace</span></Button></div>{continuationError && <p className="mt-2 text-xs text-destructive">{continuationError}</p>}</section>}
      {run.tasks.map((task) => <section key={task.id} className="border-t border-border/50 pt-4"><div className="flex items-start gap-2"><SquareTerminal className="mt-0.5 size-3.5 text-muted-foreground" /><div className="min-w-0 flex-1"><p className="text-xs font-medium">Continuation {task.sequence}</p><p className="mt-0.5 text-[11px] text-muted-foreground">{task.instruction}</p></div><span className="text-[11px] text-muted-foreground">{task.status}</span>{active(task.status) && <Button size="xs" variant="ghost" onClick={() => void cancelTask(task.id)}><X className="mr-1 size-3" />Cancel</Button>}</div><div className="mt-3 rounded-lg border border-border/60 bg-muted/[0.18] p-3 font-mono text-[11px] leading-relaxed">{task.commandPlan.map((step, index) => <div key={`${task.id}:${index}`} className="mb-2 last:mb-0"><span className="text-muted-foreground/60">{String(index + 1).padStart(2, '0')}</span> $ {step.command}<div className="ml-5 text-muted-foreground">{step.description}</div></div>)}</div><div className="mt-2"><TerminalEvents events={task.events} empty="" /></div>{task.error && <p className="mt-2 text-xs text-destructive">{task.error}</p>}<FileLinks runId={run.id} slug={slug} files={task.files} /></section>)}
...
function FileLinks({ runId, slug, files, empty }: { runId: string; slug: string; files: Array<{ id: string; fileId: string | null; name: string; sizeBytes: number }>; empty?: string }) {
  ...
  return <div className="space-y-1">{files.map((file) => <a key={file.id} href={file.fileId ? `/api/workspace-runs/${encodeURIComponent(runId)}/files/${encodeURIComponent(file.id)}?slug=${encodeURIComponent(slug)}` : undefined} className="flex items-center gap-2 rounded-md px-2 py-1.5 text-xs hover:bg-muted"><FileText className="size-3.5" />{file.name}<span className="ml-auto text-muted-foreground">{Math.max(1, Math.ceil(file.sizeBytes / 1024))} KB</span></a>)}</div>;
}
