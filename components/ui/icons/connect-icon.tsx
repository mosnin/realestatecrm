...
interface ConnectIconProps extends HTMLAttributes<HTMLDivElement> {
  size?: number;
  /** Auto-loop the connect/disconnect motion. Use as a status indicator. */
  autoPlay?: boolean;
}
...
const ConnectIcon = forwardRef<ConnectIconHandle, ConnectIconProps>(
  ({ onMouseEnter, onMouseLeave, className, size = 28, autoPlay, ...props }, ref) => {
    ...
    // Status-indicator: oscillate between connected (animate) and apart (normal)
    // so the icon reads as "linking up" continuously. The same spring transition
    // each direction keeps it readable.
    useEffect(() => {
      if (!autoPlay) return;
      let cancelled = false;
      const loop = async () => {
