...
export function CardBody({ className, ...props }: CardProps) {
  return (
    <div
      role="group"
      className={cn("flex flex-col space-y-1.5 border-t border-border p-4", className)}
      {...props}
    />
  );
}
...
const EllipseGradient = ({ color }: { color: string }) => (
  <div className="absolute inset-0 z-[5] flex h-full w-full items-center justify-center">
    <svg width="100%" height="180" viewBox="0 0 356 180" preserveAspectRatio="xMidYMid meet" fill="none" xmlns="http://www.w3.org/2000/svg">
      <rect width="356" height="180" fill="url(#paint_line_1)" />
      <defs>
        <radialGradient id="paint_line_1" cx="0" cy="0" r="1" gradientUnits="userSpaceOnUse" gradientTransform="translate(178 98) rotate(90) scale(98 178)">
...
const Layer2 = ({ color }: LayerProps) => (
  <div className="absolute top-0 left-[-1px] h-full w-full">
    ...
    {/* Fade mask slides away on hover to reveal bar chart */}
    <div className="ease-[cubic-bezier(0.6,0.6,0,1)] absolute inset-0 z-[3] transform bg-gradient-to-r from-transparent from-0% to-card to-15% transition-transform duration-500 group-hover/animated-card:translate-x-full" />
  </div>
);
...
const Layer3 = ({ color, secondaryColor }: LayerProps) => (
  ...
  >
    <div className="flex shrink-0 items-center rounded-full border border-border bg-card/25 px-1.5 py-0.5 backdrop-blur-sm transition-opacity duration-300 ease-in-out group-hover/animated-card:opacity-0">
      ...
      <span className="ml-1 text-[10px] text-foreground">Qualified</span>
    </div>
    <div className="flex shrink-0 items-center rounded-full border border-border bg-card/25 px-1.5 py-0.5 backdrop-blur-sm transition-opacity duration-300 ease-in-out group-hover/animated-card:opacity-0">
      <div className="h-1.5 w-1.5 rounded-full bg-[var(--secondary-color)]" />
      <span className="ml-1 text-[10px] text-foreground">Pending</span>
    </div>
  </div>
);
  ...
  <div className="group relative h-full w-full">
    <div className="ease-[cubic-bezier(0.6,0.6,0,1)] absolute inset-0 z-[7] flex w-full -translate-y-full items-start justify-start bg-transparent p-4 transition-transform duration-500 group-hover/animated-card:translate-y-0">
      <div className="ease-[cubic-bezier(0.6,0.6,0,1)] rounded-md border border-border bg-card/25 p-1.5 opacity-0 backdrop-blur-sm transition-opacity duration-500 group-hover/animated-card:opacity-100">
        <p className="mb-1 text-xs font-semibold text-foreground">Intake pipeline</p>
        <p className="text-xs text-muted-foreground">Weekly qualified lead volume.</p>
      </div>
    </div>
  </div>
);
