...
/**
 * FeatureCarousel — the auto-transitioning feature showcase (the key
 * interaction, reference-matched).
 *
 * A tabbed showcase that AUTO-ADVANCES on a timer. Each tab carries a progress
 * bar that fills over its duration, then transitions to the next tab. Layout:
 ...
 * for the active tab (so the bar fill is smooth and we can pause/reset it
 * precisely). When progress reaches 1 the active index advances. The bar width
 * is bound to that progress value; the panel + copy cross-fade via
 * AnimatePresence. Copy is Chippi/real-estate.
 */
                ...
                return (
                  ...
                  >
                    ...
                    <span aria-hidden className="absolute inset-x-0 top-0 h-px bg-white/10">
                      <span
                        className="block h-full bg-[#ff7a45]"
                        style={{
