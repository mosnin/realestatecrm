...
## Brokerage request isolation

Brokerage CRM pages now bind their selected brokerage in `?brokerage=<id>`. Middleware carries this selection into authenticated request context and canonicalizes ordinary brokerage links. Existing fetch callers inherit the selected brokerage from their same-origin dashboard referrer. Explicit API callers can supply the same query parameter. Cookies only select a fresh landing; API requests without a request selection fail closed. Malformed, duplicated, foreign-referrer, and legacy unbound-tab selections do not fall back to another membership. Explicit recovery reopens an available brokerage and binds it into the URL.

Both brokerage permission helpers still query current membership and role. A URL is a selector, never an authorization grant. Switching, default/chat landing redirects, member redirects, missing-realtor redirects, and billing redirects preserve the selected brokerage. Removed selections receive an unavailable state. This change does not modify sharing grants, customer records, billing, or provider deployments.

Browser evidence: two synthetic Chrome tabs using the application request resolver independently recorded A and B test writes after their shared cookie changed. Ordinary Deals navigation in A retained A. Behavioral tests exercise the actual middleware, permission helpers, revocation/downgrade behavior, redirect destinations, and malformed/missing context. This is not authenticated production acceptance. Clients that omit both the brokerage query parameter and the same-origin dashboard referrer must add explicit context; they cannot rely on a shared cookie for API authorization.

Team sharing remains a separate data model change. Current teams have membership and shared operational work, while `queryWorkforceCrm` denies team CRM reads. The next implementation must grant selected records explicitly, retain their original space ownership, recheck live membership plus grants on every read/action, and revoke access when either grant or membership is removed. Granting an entire member's private workspace merely because they joined is not acceptable. Production schema rollout follows docs/RELEASE.md.

Brokerage isolation validation: 795 test files passed, 6,774 tests passed and seven skipped; TypeScript and lint passed (existing warnings), tenant scanner passed across 962 files/127 registered tables, and 55 script contract tests passed. The earlier source-text redirect assertion was replaced with executable routing coverage.

## Explicit team CRM sharing

The selected-record sharing boundary is now implemented behind `CHIPPI_TEAM_CRM_ENABLED`: a team Shared records page, source-owner candidate selection, explicit share/revoke API, append-only grant migration, live authorization checks, and Chippi's restricted `list_shared_records` catalog/query. The exact fields, exclusions, verification limits, and production workflow are documented in `docs/product-rebuild/team-record-sharing.md`. This implements shared reads; it does not grant team-wide editing rights or complete the ownership/handoff queue architecture. No production migration or flag activation was performed.
