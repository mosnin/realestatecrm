...
| `NEXT_PUBLIC_ONBOARDING_V2` | `app/setup/page.tsx` | Feature flag: when `'true'`, new realtors get the V2 storytelling onboarding (typing-draft reveal). Unset/anything-else serves the legacy flow. `?legacy=1` on /setup forces legacy regardless. | **Low** | Realtors get the legacy onboarding flow (fully functional); the V2 storytelling reveal stays dark |
