 ...
 * checked "not in the past" + "no conflict" — so a crafted POST could book an
 * off-hours / blocked-date / wrong-weekday tour onto the real calendar.
 *
 * This helper reproduces the EXACT slot-determination logic from
 * available/route.ts (timezone-aware day math, override expansion incl.
 * recurrence, weekday + blocked-date gating, hour window, and duration-aligned
 * slot stepping with the end-of-window fit check) and returns whether the
 * requested timestamp is one of the slots that endpoint would have offered.
 *
 * It deliberately does NOT consider double-booking / GCal conflicts — those are
 * handled separately by the atomic RPC. This is purely the window check.
 */
...
export async function validateTourSlot(
  ...
  if (!effectiveOverride) {
    for (const o of overridesRaw ?? []) {
      ...
      let lands = false;
      // Bound the walk; 14-day availability horizon means recurrences never need
      // to expand far, but cap iterations defensively.
