/**
 * Durable scheduled-workflow occurrence protocol (construction-only).
 *
 ...
 * this protocol for consequential providers until that boundary is verified.
 * The legacy workflow cron remains the only runtime path.
 */
...
export const DURABLE_SCHEDULE_OCCURRENCE_RUNTIME_BLOCKERS = [
  'occurrence_executor_missing',
  'workflow_step_resume_not_integrated',
  'opaque_lease_token_missing',
  'provider_idempotency_unverified',
] as const;

export function durableScheduleOccurrenceRuntimeReadiness() {
  return {
    enabled: false as const,
    stage: 'construction_only' as const,
    blockers: DURABLE_SCHEDULE_OCCURRENCE_RUNTIME_BLOCKERS,
  };
}

/**
 * Reserved compatibility seam. It must remain false even if an environment
 * still contains the historical rollout flag: no occurrence executor is wired,
 * and policy configuration alone cannot establish runtime safety.
 */
export function durableScheduleOccurrencesEnabled(): boolean {
  return durableScheduleOccurrenceRuntimeReadiness().enabled;
}
...
// ── Typed RPC construction seam (no runtime caller) ──────────────────────
