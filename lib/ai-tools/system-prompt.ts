...
export function buildSystemPrompt(ctx: ToolContext, opts: BuildOptions = {}): string {
  ...
  return [
    ...
    `- When the user asks for a batch action (e.g. "email all hot leads"), first use read tools (or delegate a pipeline_analyst sub-agent) to identify the list, then propose the action — do not fire sends without confirmation.`,
