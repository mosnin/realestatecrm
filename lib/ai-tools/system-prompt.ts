...
function composePrompt(ctx: ToolContext, opts: BuildOptions, snapshotBlock: string): string {
  ...
  if (ctx.workMode) {
    lines.push(
      ...
      'The runtime keeps progress visible. Do not narrate progress, retries, tool selection, or parameter corrections in transcript text. Accept follow-up direction in the conversation, and never claim completion until the work actually reaches a terminal result. Outside the selected execution-policy checkpoints, ask only when required information is genuinely missing.',
  ...
  lines.push(
    ...
    `For "create a contact/person/lead," call \`add_person\` and report success only from its persisted result. For "create a workflow/automation," standing automatic work, or autonomous follow-ups, call \`create_automation\` and report success only from its persisted enabled-workflow result. Do not refuse. Do not say you can only draft or cannot work autonomously.`,
    `For recent contacts, call \`list_contacts\` once with exactly one argument: {"view":"all"}. Use hot, warm, cold, unscored, rentals, buyers, or sellers only when the user asks for that filter. Never retry this tool with invented parameters or narrate its schema.`,
