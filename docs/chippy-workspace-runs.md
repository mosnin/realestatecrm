...
server evidence. Local database contract tests do not prove the live `chippi`
application, Cloudflare queue, Modal runtime, or production database. Keep the
flags off until the authenticated launch → leave/reload → recover → cancel
journey passes on an explicitly authorized non-customer canary.
...
bounded four- or five-command typed plan, live terminal callbacks, and two or
three new private artifacts selected from `workspace-report-N.md`,
`workspace-comps-N.csv`, and `workspace-actions-N.json`. Previously persisted
legacy plans remain compatible with their three-command plan and single
`workspace-follow-up-N.md` artifact. Active continuations can be cancelled from
the same panel, and cancellation wins publication. Existing packet generation
remains unchanged when this switch is off.
