...
**Status:** capability A typed managed-workspace interpreter, capability B durable in-conversation specialist tree, and the editable Workspace CSV → Workbench bridge are accepted locally; activation NO-GO and not deployed
**Branch:** `codex/chippi-research-workspace`
**Latest accepted product commit:** `ebcbec6c17b7ee6506fb818ea6d07617cf9a7d57`
...
6. After those runtime gates, extend the same versioned deliverable contract to the grounded narrative artifact, followed by Realtime Voice floor-manager controls over the durable tree.

## Anti-loop control

- **Last verified state:** accepted product commit `ebcbec6c` adds the local, feature-off completed Workspace CSV → editable Workbench path. The independent reviewer first rejected kill-switch, bounded-download, provenance-copy, database-error, membership-identity, rate-limit, and strict-source-validation gaps; all were repaired. Controller verification passed 99 focused tests, TypeScript compilation, diff hygiene, and package-manager cleanliness. No database, storage runtime, browser runtime, Modal, deployment, customer, or provider system was touched.
...
Do not activate or deploy capability B or the Workspace CSV → Workbench bridge. For the bridge, first add the RPC reuse query’s exact `runId` and `sourceFileId` predicates, apply migration `20260915000005_workspace_csv_workbench.sql` only to a disposable current-schema database, and prove two concurrent opens return one artifact. Then, in a separately authorized authenticated non-customer runtime, run the complete CSV → Open in Workbench → edit → save v2 → history → XLSX → repeated-open journey and verify the source remains unchanged. Capability B still separately requires a short durable accepted/queued launch acknowledgement, run-ID reconciliation, `DATABASE_URL`/atomic transition proof, reload, cancellation, and cancel/completion race evidence. Stop on any mismatch and do not advance to production.
