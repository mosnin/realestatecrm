...
- **Recurring jobs** — a single `*/5` scheduled trigger fires the Worker;
  it matches every job in `worker/src/schedule.ts` against the current UTC
  instant (`src/cron-match.ts`) and enqueues one **Cloudflare Queues**
  message per due job. Per-job messages mean per-job retries (exponential
  backoff, `max_retries`), and exhausted jobs land in the `chippi-dlq`
  dead-letter queue instead of vanishing. Each tick invokes the app route,
  which owns the actual work, its kill-switches, and its Sentry monitoring.
...
- `npx wrangler tail` shows `master tick … enqueued N/23 jobs` within 5
  minutes, then `tick cron-lead-sla ok` etc.
