...
themselves use `IF NOT EXISTS` / idempotent patterns (see `docs/DATABASE.md`).
...
- For a deeper structural check, `supabase db diff --linked --schema public`
  ...
  it into a new, properly timestamped forward migration (per `docs/DATABASE.md`,
  never edit an applied migration) and apply that.
...
append-only forward migrations by design (`docs/DATABASE.md`), so "rollback"
means rolling *forward* with a corrective migration, not reversing one.
...
- **A destructive change (dropped column/table) you need back:** forward
  ...
  migration history. See `docs/BACKUP-RESTORE.md`.
...
## Known wrinkle: the legacy `003_*` migration

`supabase/migrations/003_tours_applications_pro.sql` predates the
`YYYYMMDDHHMMSS_*` timestamp convention the rest of the tree uses. The Supabase
CLI parses the leading numeric segment as a version, so it will treat `003` as
an extremely old version that sorts before everything else — which is harmless
for ordering but means the CLI's `migration list` may show it oddly, and a fresh
`db push` against a project that already has this table will rely on the file's
`IF NOT EXISTS` guards to no-op.

Do **not** rename or delete it as part of wiring up this pipeline (renaming an
applied migration is itself a drift event). If `db push`/`migration list`
chokes on it, the correct fix is a follow-up that reconciles migration history
with `supabase migration repair` against prod — a deliberate, separately
