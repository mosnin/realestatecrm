-- ============================================================================
-- Contact de-duplication / merge — transactional, space-scoped, idempotent
-- ============================================================================
-- Adds:
--   1. Contact.mergedIntoId  — audit breadcrumb. When a duplicate contact is
--      merged into a survivor we DELETE the duplicate row (re-pointing every
--      reference first), so mergedIntoId is normally write-once-then-gone. It
--      exists so a future "soft-merge / tombstone" mode, or a partial/aborted
--      merge, leaves a forward pointer instead of a dangling id. It is the only
--      change to the Contact core schema (an optional audit field).
--
--   2. merge_contacts(...) RPC — re-points ALL references from one or more
--      duplicate contacts onto a survivor, merges field values (survivor wins;
--      blanks filled from a duplicate; tags/properties unioned), records an
--      audit row, then deletes the duplicates. The whole thing runs inside the
--      function's implicit transaction, so it is ALL-OR-NOTHING: if any
--      re-point fails the duplicate is never deleted and nothing is orphaned.
--
-- Safety posture (mirrors offboard_brokerage_member):
--   * SECURITY DEFINER + `SET search_path = public`.
--   * EXECUTE granted to service_role ONLY — the API route (service-role key)
--     is the single entry point. authenticated/anon cannot call it directly,
--     so the API-level space-ownership check can't be bypassed.
--   * Space-scoped: the function refuses to merge unless EVERY contact (survivor
--     + duplicates) belongs to p_space_id. A cross-space id aborts the merge.
--
-- Idempotent migration: CREATE OR REPLACE / ADD COLUMN IF NOT EXISTS / guarded
-- index. Re-running is a no-op.
-- ============================================================================

-- ── 1. Audit breadcrumb column ──────────────────────────────────────────────
ALTER TABLE "Contact"
  ADD COLUMN IF NOT EXISTS "mergedIntoId" text REFERENCES "Contact"(id) ON DELETE SET NULL;

-- Partial index — only the (rare) tombstoned rows. Lets a future "show merged"
-- view find survivors cheaply without bloating the hot path.
CREATE INDEX IF NOT EXISTS idx_contact_merged_into
  ON "Contact"("mergedIntoId") WHERE "mergedIntoId" IS NOT NULL;

-- ── 2. The merge function ───────────────────────────────────────────────────
-- p_survivor_id : the contact that SURVIVES (keeps its id).
-- p_duplicate_ids : text[] of contacts to fold into the survivor and delete.
-- p_space_id    : the caller's workspace. Every id MUST be in this space.
-- p_actor_clerk_id : Clerk user id of the operator, for the audit row (nullable).
--
-- Returns json: { survivorId, merged: [...ids actually merged], repointed: {table:count},
--                 alreadyMerged: bool }.
--
-- Idempotency: a duplicate id that no longer exists (already merged/deleted) is
-- silently skipped — re-running the same call after success returns
-- alreadyMerged=true with an empty merged[].
...
BEGIN
  ...
  -- PK collisions) is excluded here and handled explicitly; soft-link tables
  -- (no FK) are also handled explicitly.
  ...
  FOR v_fk IN
    ...
    WHERE con.contype = 'f'
      ...
      AND rel.relname NOT IN ('Contact', 'DealContact')
  LOOP
...
-- Lock the function down to server-side callers only.
