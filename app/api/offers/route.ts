 ...
 * spaceId is NEVER read from the request body. See lib/offers.ts for the
 * scoped query implementation.
 */
...
import { listOffers, createOffer, OFFER_STATUSES } from '@/lib/offers';
...
export async function GET(_req: NextRequest) {
  ...
  try {
    const offers = await listOffers(space.id);
    return NextResponse.json(offers);
  } catch (err) {
    logger.error('[offers/GET] list failed', { spaceId: space.id }, err);
