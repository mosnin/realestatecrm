...
export async function POST(req: NextRequest) {
  ...
  const { data: task } = await supabase.from('WorkspaceRunTask').select('id,runId,sequence,status,cancellationRequestedAt,executionPlan').eq('id', taskId).eq('spaceId', spaceId).maybeSingle();
