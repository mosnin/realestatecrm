...
export async function GET(req: Request) {
  ...
  if (!kvUrl || !kvToken) {
    return NextResponse.json({ health: 'unknown', reason: 'Redis not configured' }, { status: 503 });
  }
  ...
  if (!res.ok) return NextResponse.json({ health: 'unknown', reason: 'Redis error' }, { status: 503 });
  ...
  return NextResponse.json({ health, total, dedupeRate, modalRate, windowSize: 50 }, { status });
}
