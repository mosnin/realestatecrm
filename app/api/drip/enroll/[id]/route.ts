...
export async function DELETE(_req: NextRequest, { params }: Params) {
  ...
  const { data: existing, error: lookupErr } = await tenantTable(supabase, 'DripEnrollment', { spaceId: space.id })
    .select('id, status')
    .eq('id', id)
    .maybeSingle();
  if (lookupErr) {
  ...
  const { error } = await tenantTable(supabase, 'DripEnrollment', { spaceId: space.id })
