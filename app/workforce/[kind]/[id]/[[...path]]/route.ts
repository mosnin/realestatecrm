...
export async function GET(request: Request, context: { params: Promise<{ kind: string; id: string; path?: string[] }> }) {
  ...
  try {
    ...
    const config = JSON.stringify({ workspaces, basePath, apiBase: `/api/workforce/${kind}/${encodeURIComponent(id)}`, crmHref: scope.crmHref, name: scope.principal.name, role: scope.principal.role === 'admin' ? 'Brokerage admin' : (kind === 'brokerage' ? 'Brokerage owner' : 'Personal workspace') }).replace(/</g, '\\u003c');
