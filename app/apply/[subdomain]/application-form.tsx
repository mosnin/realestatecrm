    ...
    <div className="space-y-5">
      <FieldGroup>
        ...
        <Input
          ...
          placeholder="123 Maple Street, Unit 4B"
        />
        ...
        <div className="flex flex-wrap gap-2">
          {leaseOptions.map((opt) => (
            <SelectChip
              key={opt}
              label={opt}
              selected={data.leaseTerm === opt}
              onClick={() => update('leaseTerm', data.leaseTerm === opt ? '' : opt)}
            />
          ))}
        </div>
      </FieldGroup>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <FieldGroup>
          ...
          <Input
            ...
            maxLength={4}
            value={data.idLast4}
            onChange={(e) => update('idLast4', e.target.value.replace(/\D/g, '').slice(0, 4))}
            placeholder="0000"
          />
          <FieldHint>Optional — used for pre-screening only</FieldHint>
        </FieldGroup>
      </div>
    </div>
  );
    ...
    <div className="space-y-5">
        ...
        <div className="flex flex-wrap gap-2">
          {statusOptions.map(({ value, label }) => (
            <SelectChip
              key={value}
              label={label}
              selected={data.housingStatus === value}
              onClick={() => update('housingStatus', data.housingStatus === value ? '' : value)}
            />
          ))}
        </div>
      </FieldGroup>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <FieldGroup>
          ...
          <Input
            ...
            onChange={(e) => update('currentMonthlyPayment', e.target.value)}
            placeholder="$1,800"
          />
        </FieldGroup>
        <FieldGroup>
          ...
          <StyledSelect
            value={data.lengthOfResidence}
            onChange={(v) => update('lengthOfResidence', v)}
            placeholder="Select"
          >
      ...
      <FieldGroup>
        ...
        <StyledSelect
          ...
          onChange={(v) => update('reasonForMoving', v)}
        ...
        <div className="flex flex-wrap gap-2">
          {statusOptions.map(({ value, label }) => (
            <SelectChip
              key={value}
              label={label}
              selected={data.employmentStatus === value}
              onClick={() => update('employmentStatus', data.employmentStatus === value ? '' : value)}
            />
          ))}
        </div>
      </FieldGroup>
      ...
      <FieldGroup>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <Input
            ...
            onChange={(e) => update('additionalIncomeSource', e.target.value)}
            placeholder="Source (e.g. freelance)"
          />
        </div>
        <FieldHint>Only include if you want it counted toward your application</FieldHint>
      </FieldGroup>
    </div>
  );
}
  ...
  return (
    <div className="space-y-5">
      <div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <FieldGroup>
            ...
            <Input
              value={data.currentLandlordName}
              onChange={(e) => update('currentLandlordName', e.target.value)}
          ...
          <FieldGroup>
            ...
            <Input
              type="tel"
              value={data.currentLandlordPhone}
              onChange={(e) => update('currentLandlordPhone', e.target.value)}
              placeholder="(555) 000-0000"
            />
          </FieldGroup>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <FieldGroup>
            ...
            <Input
              ...
              onChange={(e) => update('previousLandlordName', e.target.value)}
              placeholder="Landlord name"
            />
          </FieldGroup>
          <FieldGroup>
            ...
            <Input
              ...
              onChange={(e) => update('previousLandlordPhone', e.target.value)}
              placeholder="(555) 000-0000"
            />
          </FieldGroup>
        </div>
      ...
      <FieldGroup>
        ...
        <Input
          value={data.monthlyRentPaid}
          onChange={(e) => update('monthlyRentPaid', e.target.value)}
          placeholder="$1,800"
        />
      </FieldGroup>

      <YesNoField
        label="Any late payments or lease violations?"
        hint="In the past 5 years"
        value={data.hasLatePayments}
        onChange={(v) => update('hasLatePayments', v)}
      />
      ...
      {data.hasLatePayments === 'Yes' && (
        <FieldGroup>
          ...
          <Textarea
            value={data.latePaymentDetails}
            onChange={(e) => update('latePaymentDetails', e.target.value)}
      ...
      <YesNoField
        label="May we contact your references?"
        hint="Your landlord(s) may be contacted as part of screening"
        value={data.referencePermission}
        onChange={(v) => update('referencePermission', v)}
        required
      />
    </div>
  );
    ...
    <div className="space-y-5">
      <div className="flex items-start gap-3 p-3 rounded-lg bg-muted/50 border border-border">
        ...
        <p className="text-xs text-muted-foreground leading-relaxed">
          These questions are required by law in most jurisdictions. Honest answers help us find the right fit. Answering &quot;Yes&quot; does not automatically disqualify you.
        </p>
      </div>

      <YesNoField
        label="Have you ever been evicted?"
        value={data.priorEvictions}
        onChange={(v) => update('priorEvictions', v)}
        required
      />

      <YesNoField
        label="Do you have any outstanding balances owed to a landlord?"
        value={data.outstandingBalance}
        onChange={(v) => update('outstandingBalance', v)}
        required
      />
      ...
      <YesNoField
        label="Will anyone in the household smoke on the premises?"
        value={data.smoking}
      ...
      {data.pets === 'Yes' && (
        <FieldGroup>
          ...
          <Input
            ...
            placeholder="e.g. 2 cats, 1 dog (35 lbs, golden retriever)"
          />
        </FieldGroup>
      )}
      ...
      <FieldGroup>
        <div className="flex items-start gap-3">
          <input
            ...
            onChange={(e) => update('backgroundAcknowledgment', e.target.checked ? 'yes' : '')}
...
function Step8({
  ...
  const docItems = [
    { key: 'ID (driver\'s license, passport, or state ID)', always: true },
    { key: 'Pay stubs or bank statements (last 2–3 months)', always: true },
    { key: 'Offer letter (if starting a new job)', condition: data.employmentStatus === 'employed' },
    { key: 'Pet documentation (vaccination records, breed info)', condition: data.pets === 'Yes' },
  ];

  const relevant = docItems.filter((d) => d.always || d.condition);
    ...
    <div className="space-y-5">
      <div className="flex items-start gap-3 p-3 rounded-lg bg-accent border border-accent-foreground/10">
        ...
        <p className="text-xs text-accent-foreground leading-relaxed">
          Based on your answers, here&apos;s what we&apos;ll need. You can submit documents now or your agent will follow up with a secure link.
        </p>
      </div>
      ...
      <FieldGroup>
        <FieldLabel>Additional notes about your documents</FieldLabel>
        <Textarea
          value={data.documentsNotes}
          onChange={(e) => update('documentsNotes', e.target.value)}
          placeholder="Let your agent know if any documents are unavailable or need explanation..."
          rows={3}
        />
        <FieldHint>Your agent will follow up with a secure upload link after you submit.</FieldHint>
      </FieldGroup>
    </div>
  );
}
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-4">
        <div className="flex items-start gap-3">
          <input
            ...
            onChange={(e) => update('consentToScreening', e.target.checked ? 'yes' : '')}
            className="mt-1 h-4 w-4 rounded border-input accent-primary cursor-pointer"
          />
        ...
        <div className="flex items-start gap-3">
          <input
            ...
            onChange={(e) => update('truthfulnessCertification', e.target.checked ? 'yes' : '')}
            className="mt-1 h-4 w-4 rounded border-input accent-primary cursor-pointer"
          />
      ...
      <FieldGroup>
        ...
        <Input
          ...
          onChange={(e) => update('electronicSignature', e.target.value)}
      ...
      <div className="p-3 rounded-lg border border-border bg-background text-xs text-muted-foreground leading-relaxed">
        By submitting this application, you confirm that the information provided is accurate to the best of your knowledge and may be reviewed for rental screening and follow-up. Your information is handled according to our{' '}
    ...
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        ...
        <div className="text-right">
          <p className="text-xs font-medium text-foreground">Step {step} of {STEPS.length}</p>
    ...
    <div className="text-center space-y-6 py-4">
      ...
      <div className="space-y-2">
        ...
        <p className="text-muted-foreground text-sm leading-relaxed max-w-sm mx-auto">
          Your application has been submitted successfully. The agent will review your information and follow up with next steps shortly.
        </p>
      </div>
...
export function ApplicationForm({
  ...
  useEffect(() => {
    try {
      ...
      if (raw) {
        const saved = JSON.parse(raw) as { step: number; data: ApplicationData };
        if (saved?.data) {
          setData(saved.data);
  ...
  const autosave = useCallback(
    (currentStep: number, currentData: ApplicationData) => {
      setSaveIndicator('saving');
      try {
        localStorage.setItem(storageKey, JSON.stringify({ step: currentStep, data: currentData }));
      } catch {
  ...
  function validate(s: number): boolean {
    ...
    if (s === 1 && !data.propertyAddress.trim()) errs.propertyAddress = 'Property address is required';
    if (s === 2) {
    ...
    if (s === 5 && !data.monthlyIncome.trim()) errs.monthlyIncome = 'Monthly income is required';
    if (s === 6 && !data.referencePermission) errs.referencePermission = 'Please answer this question';
    if (s === 7 && !data.backgroundAcknowledgment) errs.backgroundAcknowledgment = 'Please acknowledge to continue';
    if (s === 9) {
    ...
    <div className="min-h-screen flex flex-col bg-[#f5f6f8]">
      ...
      <header className="w-full border-b border-border bg-white/80 backdrop-blur-sm sticky top-0 z-20">
        <div className="max-w-2xl mx-auto px-4 h-14 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2 text-xs text-muted-foreground">
            ...
            {saveIndicator === 'saved' && (
              <>
                <CloudCheck size={12} className="text-primary" />
      ...
      <main className="flex-1 w-full max-w-2xl mx-auto px-4 py-8 space-y-5">
        ...
        >
          ...
          {step === 9 && <Step9 data={data} update={update} spaceName={spaceName} />}
        ...
        {!isLastStep && step < STEPS.length - 1 && (
          <p className="text-center text-xs text-muted-foreground">
            {step === 1 && 'Next, we\'ll ask about you personally.'}
            {step === 2 && 'Next, let\'s look at your current living situation.'}
            {step === 3 && 'Next, who will be living with you?'}
            {step === 4 && 'Next, let\'s review your income.'}
            {step === 5 && 'Next, your rental history.'}
            {step === 6 && 'Almost done — a few quick background questions.'}
            {step === 7 && 'Next, document uploads.'}
          </p>
        )}
      </main>
