...
export function SharedRecordsClient({teamId,teamName}:{teamId:string;teamName:string}) {
  ...
  useEffect(()=>{
    ...
    fetch(`${endpoint}?kind=${kind}&offset=${offset}`,{signal:controller.signal,cache:'no-store'}).then(async response=>{
      ...
      const data=await response.json();if(controller.signal.aborted)return;setRecords(data.records);setNext(data.nextOffset);setBrokerageAvailable(Boolean(data.brokerageSharingAvailable));
  ...
  return <main className="app-theme min-h-screen bg-background text-foreground"><div className="mx-auto max-w-5xl px-5 py-10 sm:px-8">
    ...
    <header className="my-8 flex flex-wrap items-start justify-between gap-4"><div><p className="text-sm text-muted-foreground">{teamName}</p><h1 className="mt-1 text-2xl font-semibold">Shared records</h1><p className="mt-2 max-w-xl text-sm text-muted-foreground">Selected records your team and Chippi can read. Originals stay with their owner.</p></div><div className="flex flex-wrap gap-2"><Button variant="outline" asChild><Link href={`/workforce/team/${encodeURIComponent(teamId)}/app`}>Open Chippi</Link></Button><Button onClick={()=>{setAdding(true);setSelected(null);}} disabled={busy}>Share a record</Button></div></header>
